...
def split_train_val_test_len(len_alldata, val_percent, test_percent):
...
def mix_external_data(timestamps, holiday_fname='./TaxiBJ/BJ_Holiday.txt',
                      meteoraol_fname='./TaxiBJ/BJ_Meteorology.h5'):
    mix_data = []
...
def save():
    ...
    len_train, len_val, len_test = split_train_val_test_len(data_all_stack.shape[0], len_val_percent, len_test_percent)
    ...
    np.save(os.path.join(save_path, 'train_ts'), train_ts)
    np.save(os.path.join(save_path, 'val_ts'), val_ts)
    np.save(os.path.join(save_path, 'test_ts'), test_ts)
...
class STData(data.Dataset):
    ...
    target: [2, 32, 32]
    '''
    def __init__(self, dataset_type="train", windows_size=7*48,  dir_path='./processed/TaxiBJ/',
    ...
    def __len__(self):
        if self.dataset_type == "train":
             return len(self.train_ts)
        elif self.dataset_type == "validate":
            return len(self.validate_ts)
        elif self.dataset_type == "test":
            return len(self.test_ts)
        elif self.dataset_type == "train_all":
            return len(self.train_all_ts)
